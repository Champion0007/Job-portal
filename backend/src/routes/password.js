...
const RESET_TOKEN_EXPIRES_MIN = Number(process.env.RESET_TOKEN_EXPIRES_MIN) || 60; // minutes
...
router.post('/forgot', async (req, res) => {
  try {
    ...
    if (!user) {
      return res.json({ ok: true, message: 'A reset link sent to the registred email_id.' });
    }

    // create token (send plain token, store hashed)
    ...
    // Debug: log reset URL so you can open it directly while debugging
    console.log('[password] resetUrl ->', resetUrl);
    ...
    return res.json({ ok: true, message: 'A reset link sent to registerd email_id.' });
  } catch (err) {
    console.error(err);
